...
class DeviceView:
    ...
    def draw(self, devices: list[dict], selected: int):
        ...
        for i, device in enumerate(devices):
            is_sel = i == selected
            surf = self.fonts.menu.render(device["name"][:55], True, WHITE if is_sel else DIM)
            rect = surf.get_rect(center=(W // 2, start_y + i * row_h))
            if is_sel:
                pygame.draw.rect(self.screen, (35, 35, 55), rect.inflate(20, 8), border_radius=4)
                arrow = self.fonts.menu.render('▶', True, WHITE)
                self.screen.blit(arrow, arrow.get_rect(midright=(rect.left - 12, rect.centery)))
